...
void loop() {
  ...
  spr.drawString("Enviro Sense",50,15); //Drawing a text string 
  ...
  //Setting Microphone
  int val = analogRead(WIO_MIC); //Store sensor values 
  val = map(val,0,800,0,100); //Map sensor values 
  spr.setTextSize(2);
  spr.drawString("Microphone",160,65);
  spr.setTextSize(3);
  spr.drawNumber(val,200,95); //Display sensor values as percentage 
  spr.drawString("db",240,95);
